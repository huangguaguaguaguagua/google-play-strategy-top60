#!/usr/bin/env python3
"""Build the Beijing 2026-09-14 Google Play and Apple App Store snapshots."""
...
CAPTURE = "2026-09-14"
GOOGLE_DATE = "2026-09-14"
GOOGLE_BASELINE = "2026-06-16"
IOS_DATE = "2026-09-14"
IOS_BASELINE = "2026-06-16"

GOOGLE_HISTORY = [
    (f"data/games-{date}.json", f"data/enrichment-{date}.json", f"data/trends-{date}.json")
    for date in (
        "20260911", "20260910", "20260909", "20260908", "20260907", "20260904", "20260903",
...
def pokemon_google_profile(row, metadata, asset_rank):
    ...
    analysis = trend(
        ...
        "今日首次进入Google Play美国策略畅销榜#58，同时从iOS策略TOP60退出。美国Google商品页序列化released日期为2026-06-11，早于本次90天窗口起点2026-06-16；宝可梦官网标注移动版正式上线6月17日，两种日期口径不同。状态遵守本商店元数据规则，不与iOS Lookup上架日混用。",
...
def build_google(rows, source_info):
    current = records("data/games-20260911.json", "data/enrichment-20260911.json", "data/trends-20260911.json", "packageName")
    ...
    enrichment = deepcopy(load("data/enrichment-20260911.json"))
    ...
    save("data/games-20260914.json", games)
    save("data/enrichment-20260914.json", enrichment)
    save("data/trends-20260914.json", trends)
    ...
    save(
        "data/history/google-play/2026-09-14.json",
        {
            "store": "google-play",
            "country": "US",
...
def build_ios(rows, source_url, source_updated):
    current = records("data/ios-games-20260911.json", "data/ios-enrichment-20260911.json", "data/ios-trends-20260911.json", "appId")
    ...
    enrichment = deepcopy(load("data/ios-enrichment-20260911.json"))
    enrichment["productCompaniesByRank"] = companies
    enrichment["comparisonPolicy"].update(
    ...
    save("data/ios-games-20260914.json", games)
    save("data/ios-enrichment-20260914.json", enrichment)
    save("data/ios-trends-20260914.json", trends)
    if bundle:
    ...
    save(
        "data/history/ios/2026-09-14.json",
        {
            "store": "ios",
            "country": "US",
            "device": "iPhone",
