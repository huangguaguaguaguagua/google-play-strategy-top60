...
google_products = [
    ...
    product("Guns of Glory: Lost Island", "com.diandian.gog", "googlePlay", "up", "+7 · #30", "Google由#37升至#30，同时iOS回榜#59。两端方向一致但强度差异很大；Google已进入中段，iOS仍压线，需连续快照确认是否与同期活动有关。"),
...
ios_products = [
    product("The Battle Cats", "850057092", "ios", "entry", "回榜 · #28", "8月31日最后一次在榜为#53，本期回到#28，属于成熟产品从榜尾回到中段；Google当前无对应TOP60排名，缺少活动证据，先按iOS单端回补信号处理。"),
    ...
    product("Warline: Sniper Strike", "6742809194", "ios", "entry", "回榜 · #56", "8月24日曾位于#58，本期回到#56；按iOS 7月22日上架日期仍属于近90天新品。Google已升至#28，当前明显由Android侧承接，iOS仍有榜尾风险。"),
    product("Guns of Glory: Lost Island", "1274354704", "ios", "entry", "回榜 · #59", "9月4日从#59掉榜后再次以同一位置回归；Google同期升7位至#30。iOS反复压线，双端共同在榜但商业化强度仍由Google主导。"),
    ...
    product("Warhammer 40,000: Tacticus", "1599937506", "ios", "up", "+19 · #29", "iOS由#48升至#29，是本期最大上涨；Google同期位于#28且变化有限，双端重新靠拢到相近区间。尚无与本次周末涨幅直接对应的官方节点，先记为待验证信号。"),
    product("Dragon Traveler", "6751086804", "ios", "down", "−17 · #48", "9月4日刚从#52升至#31，本期回落到#48，前一轮高位基本回吐；Google仍无对应TOP60排名。若下一快照继续下滑，将重新接近榜尾。"),
    product("Kingdom Guard: Tower Defense", "1570095804", "ios", "up", "+16 · #36", "iOS由#52升至#36，但Google由#47降至#52，形成明确跨端分化。iPhone侧回补强，Android侧却进入榜尾风险区，不能合并为统一收入趋势。"),
    product("Forge Master – Idle RPG", "6746636289", "ios", "down", "−16 · #51", "iOS由#35降至#51，Google没有对应TOP60排名。产品从中段退回榜尾，周末累计跌幅较大，但当前缺少可核验的活动结束节点。"),
    product("War and Order", "1071744151", "ios", "up", "+14 · #25", "iOS由#39升至#25，Google也由#30升至#26，是本期最清晰的双端同步上行之一。三天累计信号尚不能外推长期趋势，下一工作日是否守住双端TOP30更关键。"),
    product("MARVEL SNAP", "1592081003", "ios", "down", "−14 · #39", "iOS由#25降至#39，而Google小升至#33，AXIS赛季后的双端走势转为分化。iOS回吐明显，Google维持中段，需避免把一个商店的跌幅扩展为全产品判断。"),
    product("Yu-Gi-Oh! Master Duel", "1554247785", "ios", "down", "−21 · #45", "9月4日借回榜来到#24，本期回落到#45，WCS同期高位没有继续维持。产品仍在榜，但三天后大幅回吐使短期赛事/付费峰值解释更可信。"),
...
news = [
    ...
    {"category": "新赛季", "date": "2026-08-31", "title": "MARVEL SNAP继续运营AXIS: Inversion赛季", "summary": "赛季包含新角色、地点、登录日历、Draft、Team Clash与多段活动，并提供网页商店季票奖励。", "impact": "今日Google #33、iOS #39，iPhone侧较9月4日回落14位；赛季首周的双端承接已出现分化。", "source": "MARVEL SNAP官方", "url": "https://marvelsnap.com/new-season-axis-inversion/"},
...
closing = [
    {"type": "判断", "title": "周末后三天累计波动主要集中在iOS中后段", "detail": "Google为2进2出，iOS为5进5出；iOS还出现Yu-Gi-Oh! −21、Tacticus +19和Dragon Traveler −17。由于这是9月4日至7日累计变化，不把幅度解释为单日冲击。"},
    {"type": "判断", "title": "War and Order形成今日最清晰的双端共同上行", "detail": "Google +4至#26、iOS +14至#25，两个商店收敛到相邻位置；相比Lords Mobile、Kingdom Guard和MARVEL SNAP的跨端分化，这一信号更值得连续验证，但仍不足以断言长期趋势。"},
    ...
    {"type": "关注", "title": "9月8日下一快照：核对五款iOS回榜产品留存", "detail": "The Battle Cats #28与Watcher #35信号强于位于#52、#56、#59的Star Trek、Warline和Guns of Glory；后三款若不能脱离榜尾，继续按短期回补处理。"},
...
brief = {
    ...
    "title": "周末后Google两进两出、iOS五进五出；War and Order双端同步上扬",
    "summary": "与9月4日上一份有效快照相比，Google Play有Forge of Empires与Last Fortress: Underground进入/回榜，Sea War与Arknights掉榜；iOS有5款回榜、5款掉榜。War and Order双端升至#26/#25，是最清晰的共同上行；Lords Mobile、Kingdom Guard与MARVEL SNAP则呈跨端分化。",
    ...
    "rankingDynamics": {
        ...
        "ios": {"label": "App Store · iPhone/iOS", "sourceTime": f"Apple RSS 北京时间 {source_date:%Y-%m-%d}", "anchors": "榜单锚点：#1 Pokémon GO · #25 War and Order · #60 Fire Emblem Heroes", "products": ios_products},
    },
    "marketNews": news,
    "closing": closing,
