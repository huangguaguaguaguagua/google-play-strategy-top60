...
google_products = [
    card("googlePlay", "Supremacy: World War 3", "entry", "9月8日、12日和今天三次在Google榜尾回归，本次为#58；iOS同时下滑3位至#44。连续进出说明Android端仍处边界波动，不能写成稳定回暖。"),
    ...
    card("googlePlay", "Game of Thrones: Conquest", "exit", "上期已降至Google #59，今天掉榜；iOS也下滑7位至#45。两端方向一致，但Google离榜仍只是一日边界信号。"),
    card("googlePlay", "Infinity Kingdom", "up", "Google升4位至#24，iOS小幅回落至同为#24；两端名次重合并共同位于前半区，不过今天没有匹配幅度的官方活动证据。"),
    card("googlePlay", "Raid Rush", "up", "Google升3位至#50，iOS则大涨17位至#22；这是今日少数两端同向信号，但幅度明显集中在iPhone端，仍需下一快照确认。"),
    card("googlePlay", "League of Legends: Wild Rift", "down", "Google下滑6位至#46，iOS继续在榜外；7.2e平衡补丁是近期节点，但没有证据把名次变化直接归因于版本。"),
    card("googlePlay", "Last Shelter: Survival", "down", "Google下滑3位至#52，iOS仍在榜外；成熟产品重新靠近榜尾，当前未发现可确认的重大转折。"),
    card("googlePlay", "Sea War", "down", "Google下滑2位至#60，iOS则从上期#56掉榜；同产品两端同步走弱，是今天最明确的共同掉榜风险，但仍需后续快照验证。"),
]


ios_products = [
    card("ios", "DC: Dark Legion", "entry", "回到iOS #47。9月9日版本加入Forge Breach联盟占点玩法，并有Beast Boy相关活动；节点与回榜相邻，但不足以确认因果。"),
    card("ios", "SimCity BuildIt", "entry", "回到iOS #49，Google策略榜未收录同产品；当前仅能确认成熟城建产品的单端回榜，未发现可确认的重大运营转折。"),
    card("ios", "Arknights", "entry", "首次进入项目保存的iOS策略TOP60，位列#58；Apple本店2020年上架，因此不是近90天新品。当前版本为8月13日强制更新，未发现与今天回榜直接对应的可确认原因。", first=True),
    card("ios", "Warline", "exit", "上期iOS #52后掉榜；Google仍小升至#29。该作按Apple本店7月22日上架日期仍属近90天产品，但今天呈现明显跨端分化。"),
    card("ios", "Sea War", "exit", "上期iOS #56后掉榜，Google也下滑至#60；两端同步走弱并触及Android榜尾，是今天最清晰的共同风险信号，但仍需下一快照确认。"),
    ...
    card("ios", "Watcher of Realms", "down", "由#47跌8位至#55，直接进入掉榜区；Google没有同口径排名，先按iPhone端榜尾风险处理。"),
    card("ios", "Rise of Kingdoms", "down", "由#28跌6位至#34，Google同产品仍在#18；成熟4X出现16位跨端差，不能用iOS单日回落概括产品整体。"),
    card("ios", "Dragon Traveler", "down", "由#40跌8位至#48，仍在上线初期的高波动阶段；Google策略榜未收录，暂按单端待验证信号。"),
    card("ios", "Game of Thrones: Conquest", "down", "iOS下滑7位至#45，Google则从#59掉榜；两端共同走弱但幅度不同，下一快照将检验是否只是短时回落。"),
    card("ios", "Kingdom Guard", "down", "由#55跌5位至#60，Google同产品也下滑2位至#53；两端同向回落且iOS压线，先按榜尾风险处理。"),
]
...
news = [
    ...
    {"category": "联盟玩法 / 版本", "date": "2026-09-09", "title": "DC: Dark Legion加入Forge Breach联盟占点玩法", "summary": "Apple版本说明显示联盟成员需要占领建筑、使用协议克制并摧毁核心；同期商品页也展示Beast Boy相关内容。", "impact": "产品今天回到iOS #47；活动节点提供解释线索，但单次回榜不足以确认增收。", "source": "Apple App Store官方商品页", "url": "https://apps.apple.com/us/app/dc-dark-legion/id6479020757"},
    {"category": "版本 / 成长系统", "date": "2026-09-15", "title": "MARVEL SNAP 57.x重做角色精通奖励轨道", "summary": "官方更新加入Wild Boosters、Split Reroll Coins、额外Custom Card槽位与Level 27奖励，并预告Draft于9月28日回归。", "impact": "该作今天iOS小升至#50、Google升至#55，两端仍靠近榜尾；新系统应作为后续回流观察点。", "source": "MARVEL SNAP官方", "url": "https://marvelsnap.com/patch-notes-september-15-2026/"},
    ...
    {"category": "版本 / 跨端", "date": "2026-09-09", "title": "Wild Rift 7.2e调整Vi、Swain及多件装备", "summary": "Riot削弱Vi、Swain，并增强Janna、Nautilus、Malphite等英雄，同时修改多件装备。", "impact": "该作今天Google降至#46且iOS仍在榜外；补丁只是背景，尚无官方材料证明与名次存在因果。", "source": "Riot Games官方", "url": "https://wildrift.leagueoflegends.com/en-us/news/game-updates/wild-rift-patch-notes-72e/"},
...
closing = [
    {"type": "判断", "title": "iOS波动显著高于Google，榜首也再次易位", "detail": "Google两进两出、iOS三进三出；Google 58款共同在榜产品中有49款日变动不超过2位、中位绝对变动1位，iOS 57款共同在榜产品中只有31款不超过2位、中位绝对变动2位，并由Pokémon GO重回榜首。"},
    ...
    {"type": "判断", "title": "新品进入榜尾，成熟产品跨端风险并存", "detail": "Age of Spirits美国区上架次日首次进入Google #59，Arknights则以成熟产品身份首次进入项目iOS榜#58；Sea War在iOS掉榜且Google压至#60，Game of Thrones: Conquest也两端共同走弱。"},
    {"type": "关注", "title": "9月18—21日：验证Age of Spirits首发承接", "detail": "若Age of Spirits连续留榜并脱离后五名，再把它从首日进入升级为短期承接信号；若快速掉榜，则归档为首发榜尾脉冲。"},
    {"type": "关注", "title": "下一工作日：复核三组iOS大涨与Arknights回榜", "detail": "Mobile Legends需保持前20、Hearthstone保持前40、Raid Rush保持前30；Arknights若仍停留后五名或立即掉榜，只记录为成熟产品榜尾脉冲。"},
    {"type": "关注", "title": "截至9月22日：跟踪跨端榜尾与活动窗口", "detail": "重点看Sea War能否守住Google、Kingdom Guard能否守住iOS、Supremacy是否再次退出Google，并在Clash of Clans WWE活动结束前复核头部稳定度。"},
]
...
brief = {
    ...
    "rankingDynamics": {
        ...
        "ios": {"label": "App Store · iPhone/iOS", "sourceTime": f"Apple RSS 北京时间 {source_date:%Y-%m-%d}（源更新：{ios_history['sourceUpdated']}）", "anchors": "榜单锚点：#1 Pokémon GO · #25 Magic: The Gathering Arena · #60 Kingdom Guard:Tower Defense TD", "products": ios_products},
    },
    "marketNews": news,
    "closing": closing,
