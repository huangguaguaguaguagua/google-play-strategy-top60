...
function mergeData(rawGames, enrichment, assets, trends) {
  return rawGames.map((game) => {
    ...
    const finalNote = trends?.[key] || game.note?.trim() || enrichment.notesByRank?.[key] || "";
...
async function init() {
  ...
  try {
    const [rawGames, enrichment, assets, trends] = await Promise.all([
      ...
      loadAssets(),
      fetch("data/trends.json?v=" + CACHE_VERSION).then((response) => {
        if (!response.ok) throw new Error("详细趋势数据加载失败：" + response.status);
        return response.json();
      }),
    ]);
